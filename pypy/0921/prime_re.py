...
def prim(s):
    ...
    MST = [0] * V # 내가 만들어 갈 신장 트리에 포함시킨 정점 정보


    # MST[1] = 1: 1번 정점은 신장트리에 포함되어 있다.
    # MST[1] = 0: 1번 정점은 신장트리에 포함되어 있지 않다.

    heappush(heap, (0, s)) # heap에 시작정점 넣고 시작
    # 최소 신장 트리(가중치 합)
    MIN_V = 0
    ...
    while heap:
        ...
        # 정점 v가 지금 내가 만들고 있는 싱장트리에 이미 포함이 되어 있으면 continue
        if MST[v]:
            continue 
        ...
        # v와 연결된 모든 노드들을 확인 
        for u in range(V):
            # u 노드와 v 노드가 인접해 있지 않거나
            # u 노드가 이미 신장트리에 포함되어 있으면 볼 필요 없음
            if adjm[v][u] == 0 or MST[u]:
                continue 
            
            # u노드가 v노드와 인접해있고, 신장트리에 포함된 적이 없는 노드다.
