...
def dfs(s,V): # 시작점, 정점의 개수
    ...
    while True:
        ...
        # 현재 정점 i에서 다른정점 j로 갈 수 잇는 길이 있는지 어떻게 확인??
        # 인접행렬 or 인접 리스트
        # adj_m[i][j] == 1이면 길이 있다.
        for j in range(V):
            # 내가 이전에 j번 정점을 방문한 적이 있는지도 확인해야 함
            if adj_m[i][j] == 1 and visited[j] == 0:
                # j번째 정점을 방문할 것이기 때문에 이전 정점인 i로 되돌아오기 위해서 스택에 i를 추가
                stack.append(i)
                # j번째 정점에서 하고싶은 일
                print(node[j])
                # 방문했다고 처리
                visited[j] = 1
                # 현재 위치를 j로 바꾸고 다음 탐색 시작 준비
                i = j
            ...
            # 스택 비어있는지 확인 / if stack , stack원소 하나라도 있으면 true
            if stack:
                i = stack.pop()
            else:
...
# 2번 정점에서 3번 정점으로 가는 길이 없다. adj[2][3] = 0
#         A B C D E F G
...
adj_l = [[] for _ in range(V+1)] # []* 안되는거, 0은 왜 안돼?
for i in range(E):
    # 연결되려면 연결시작점 s, 연결종료e
